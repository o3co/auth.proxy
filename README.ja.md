...
知っておくべき限界が 2 つある。ブラウザはクロスオリジンではこのヘッダーを読めない — `WWW-Authenticate` は CORS セーフリストに含まれず、このプロキシは `Access-Control-Expose-Headers` を設定しない — ので、別オリジンの SPA に見えるのはステータスとボディだけである。また `invalid_token` はクライアントに新しいトークンを取得して再試行するよう促す（§3.1）が、後述の audience のケースではそれは役に立たない。`aud` がこのプロキシのクライアントも、そのクライアントの `allowedAudiences` に登録された audience も指していないためにプロバイダーが `active: false` と答えるトークンは、どれだけ新しくても同じように拒否される。
...
- **BFF 構成ではプロバイダーのアクセストークン寿命を短く保つ。** `oauth.accessToken.defaultExpiresIn`（古いプロバイダーでは `expiresIn`。新しいものは非推奨の別名として残している）がオフライン検証側でのリプレイ露出を縛る。クッキー → トークン交換はプロキシが担うので、寿命を短くしても、ブラウザセッションが有効な間に増えるのはグラント呼び出しだけである。
