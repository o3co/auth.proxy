...
The repository-level `CHANGELOG.md` carries **no** `## [Unreleased]` section on HEAD between cuts. The release-cut PR writes the whole `## [X.Y.Z] - YYYY-MM-DD` section (Keep a Changelog headings: Added / Changed / Removed / Fixed / Security) from the commit log — `git log <lastTag>..HEAD` — and its description lists that range (`git log vA..HEAD --oneline`) so the reviewer can check that every operator-visible change in it has an entry. A feature or fix PR therefore does **not** touch `CHANGELOG.md`; it says in its own description what an operator will notice, which is what the cut reads. **Do not pre-stamp** specific version numbers anywhere (e.g., `## [0.6.0]`, `## [1.0.0]`, `## [0.0.4] — Unreleased`) before the tag is cut. The cut PR's merge and the tag are separate git operations that happen as one atomic release-cut step: the tag points at the merge commit.

The rule is about the repository-level `CHANGELOG.md` — the file the cut reads and the tag describes. A package-level `CHANGELOG.md`, where a repository keeps one, follows the same convention; a standing `## [Unreleased]` still found in one predates this rule and is folded into that package's next cut section — it is legacy, not a second convention, and nothing reads it as a release. The heading's separator between version and date is whatever the repository's `CHANGELOG.md` already uses — a hyphen in most, an em dash in some; the examples in this document write a hyphen — and a release workflow that checks for the section matches on `## [X.Y.Z]` alone.
