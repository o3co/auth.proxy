...
export const createRouter = ({ config }: { config: AppConfig }): express.Router => {
  ...
  const cacheMaxEntries: number = config.auth.introspect.cacheMaxEntries;
  const introspectTimeoutMs: number = config.auth.introspect.timeoutMs;
  ...
  router
    ...
    .use(async (req: Request, res: Response, next) => {
      ...
      try {
        const result = await introspect(bearer.token, introspectUrl, cacheTtlSec, requestId, authHeader, cacheMaxEntries, introspectTimeoutMs);
        if (!result.active) {
