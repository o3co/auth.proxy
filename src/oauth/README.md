...
- validation — [`buildAuthHeader`](../modes/validation/introspect.mts), only when both `auth.validation.client` credentials are set; otherwise the inbound token itself is the introspection credential;
