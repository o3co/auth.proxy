...
- **One logger.** The routers and `exchange.mts` import the singleton (the default export of [`logger.mts`](logger.mts)) rather than receive a `Logger`; only `shutdown.mts` takes one as an option. Tests spy on the singleton. Not a replacement boundary today (F22). Output is NDJSON on stdout, level from `LOG_LEVEL` read when the logger is created ([`createProxyLogger`](logger.mts)) — the only environment read outside the HOCON parse in `app.mts`.
...
| session token cache and single-flight table | the [`createRouter`](modes/injection/router.mts) closure | at process exit |
