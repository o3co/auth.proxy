...
| [`response-body.mts`](response-body.mts) | contract | what to do with a provider response once the answer is decided without it: [`discardBody`](response-body.mts) releases a body nothing will read; beyond undici's 64 KiB read-ahead an unread body would hold its socket until collected. Both modes use it; it belongs to neither (#95 F28, F37). Pinned through its callers: [`session-grant-client.test.mts`](modes/injection/__tests__/session-grant-client.test.mts), [`jwt-bearer-client.test.mts`](modes/injection/__tests__/jwt-bearer-client.test.mts), [`introspection-client.test.mts`](modes/validation/__tests__/introspection-client.test.mts). |
| [`single-flight.mts`](single-flight.mts) | contract + in-memory implementation | [`SingleFlight`](single-flight.mts): one flight per key, the rejection shared with every waiter, the slot cleared in `finally`. Both modes coalesce with it; it belongs to neither. Its own contract is pinned by [`__tests__/single-flight.test.mts`](__tests__/single-flight.test.mts). |
