...
Non-test edges: `app` → `config/application.schema`, `app-internal`, `logger`, `router/index`, `shutdown` · `app-internal` → `modes/injection/router`, `modes/validation/router` · `modes/validation/router` → `express/bearer`, `express/requestId`, `router/upstream`, `logger`, `modes/validation/introspect` · `modes/validation/introspect` → `oauth/client-secret-basic` · `modes/injection/router` → `express/requestId`, `router/upstream`, `logger`, its siblings · `modes/injection/decision` → its siblings and the `Logger` type · `modes/injection/exchange` → `logger`, its siblings · `modes/injection/jwt-bearer-client` → `oauth/client-secret-basic`, `provider-error`, `session-grant-client` · `modes/injection/session-grant-client` → `provider-error` · `router/upstream` → `express-http-proxy`. No cycles. Direction: `modes/*` depend on `express/`, `oauth/`, `router/upstream` and `logger`; nothing under `express/`, `oauth/`, `router/` or `config/` imports a mode.
...
- **One logger.** The validation router and `exchange.mts` import the singleton (the default export of [`logger.mts`](logger.mts)) rather than receive a `Logger`; the injection router uses it as the default `deps.logger` and hands it to [`decideInjection`](modes/injection/decision.mts) as a [`Logger`](logger.mts) (#95 F4), and `shutdown.mts` takes one as an option. Tests spy on the singleton, or inject a fake where `deps` allows it. A replacement boundary on the injection *session* path only — the exchange handler still imports the singleton (F2, F22). Output is NDJSON on stdout, level from `LOG_LEVEL` read when the logger is created ([`createProxyLogger`](logger.mts)) — the only environment read outside the HOCON parse in `app.mts`.
...
| session token cache and single-flight table | the [`createRouter`](modes/injection/router.mts) closure — built there, or supplied via `deps` (#95 F4) | at process exit |
