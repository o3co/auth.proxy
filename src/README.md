...
- **The healthcheck is outside `pathPrefix` and CORS (#95 F25).** It is mounted first and at the root, so `/_healthcheck` answers whatever the prefix is, without CORS headers and without a request id: none is read, generated or echoed. It is liveness only — the process is up — and says nothing about the provider. No test imports `app.mts`, which runs at import; [`__tests__/app-entry.test.mts`](__tests__/app-entry.test.mts) runs it in a child process and pins this placement, the refusal of a bad configuration before the process listens, and CORS under the prefix. The probe's path and method are pinned by [`router/__tests__/Healthcheck.test.mts`](router/__tests__/Healthcheck.test.mts).
...
- **An Error reaches a log line only through an allowlist (#95 F48).** Some errors carry bytes nobody chose to log — undici's `HTTPParserError.data` is the unparsed rest of the provider's response, which may echo the token the proxy just sent — so an `Error` logged under the `error` or the `err` key keeps only named fields, with URL credentials redacted; everything else on it is dropped. The validation path logs under `error` and `shutdown.mts` under `err`; an Error logged under any other key gets pino's handling, not the allowlist. A value under `error` that is not an `Error` passes through unchanged: injection logs `String(err)` there for a throw it could not classify, whatever that message holds. The mechanics are in the header of [`logger.mts`](logger.mts), pinned by [`__tests__/logger.test.mts`](__tests__/logger.test.mts).
- **One log vocabulary for both modes (#134).** Every line about a request — each router's `incoming request` and every decision line — carries the request id as `requestId` and an `event` named after its mode (`injection.*`, `validation.*`), so a query on `requestId` covers both modes; the event names are per mode. The `error` field is not shared: validation logs the `Error` itself, serialised through the allowlist below into an object with `message`, `stack` and `cause`, while injection logs a string — the error's message, or `String(err)` for a throw it could not classify. A query on `error.message` matches validation lines only. The level of a provider failure says whose it was: the provider refusing the caller's credential is `info` (a refusal of what the credential may do, injection's `exchange_not_permitted`, is `warn`); the proxy's own failures — its client refused, a redirect, a provider `5xx`, a timeout, an invalid response, an unexpected throw — are `error`. One timeout is not: on the session grant, a provider `401`'s body is read to tell `invalid_client` from an expired session (#95 F47), and a timeout during that read is answered as the expired session it almost always is — `401 session_required`, logged `injection.session_unauthorized` at `info`. Pinned by each mode's `decision.test.mts` and `router.test.mts`.
