...
| [`single-flight.mts`](single-flight.mts) | contract + in-memory implementation | [`SingleFlight`](single-flight.mts): one flight per key, the rejection shared with every waiter, the slot cleared in `finally`. Both modes coalesce with it; it belongs to neither. |
