...
- **One logger.** The validation router imports the singleton (the default export of [`logger.mts`](logger.mts)) rather than receive a `Logger`; the injection router uses it as the default `deps.logger` and hands it to [`decideInjection`](modes/injection/decision.mts) and [`decideExchange`](modes/injection/exchange.mts) as a [`Logger`](logger.mts) (#95 F2, F4), and `shutdown.mts` takes one as an option. Tests spy on the singleton, or inject a fake where `deps` allows it. A replacement boundary on the injection path; not on validation (F3, F22). Output is NDJSON on stdout, level from `LOG_LEVEL` read when the logger is created ([`createProxyLogger`](logger.mts)) — the only environment read outside the HOCON parse in `app.mts`.
